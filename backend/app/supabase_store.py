...
class SupabaseStore:
    ...
    def list_credentials(self) -> list[dict[str, Any]]:
        rows = self.client.table("scoped_credentials").select(
            "credential_id,agent_id,tool,scopes,expires_at,revoked_at"
        ).order("created_at", desc=True).execute().data or []
        return [{**row, "revoked": bool(row.get("revoked_at"))} for row in rows]
    ...
    def insert_approval(self, approval: Any) -> None:
        self.client.table("approvals").insert({
            ...
            "requested_by": approval.requested_by, "request_fingerprint": approval.request_fingerprint,
    ...
    def _approval(row: dict[str, Any]):
        ...
        return Approval(
            ...
            request_fingerprint=row.get("request_fingerprint", ""), status=row["status"], decision_note=row.get("decision_note"),
            expires_at=datetime.fromisoformat(row["expires_at"].replace("Z", "+00:00")),
            decided_by=row.get("decided_by"),
            decided_at=datetime.fromisoformat(row["decided_at"].replace("Z", "+00:00")) if row.get("decided_at") else None,
            consumed_at=datetime.fromisoformat(row["consumed_at"].replace("Z", "+00:00")) if row.get("consumed_at") else None,
        )

    def get_approval(self, approval_id: str):
        rows = self.client.table("approvals").select(
            "approval_id,event_id,requested_by,request_fingerprint,status,decision_note,expires_at,decided_by,decided_at,consumed_at"
    ...
    def update_approval(self, approval: Any) -> None:
        self.client.table("approvals").update({
            "status": approval.status, "request_fingerprint": approval.request_fingerprint,
